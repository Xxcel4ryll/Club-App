...
async function render() {
  ...
  return ` 
    ...
    <section class="carousel junior">
      <main class="carousel-title">Computing Devices</main>
      ${bestSelling
        .map(
          (item, index) =>
            `
      ${
        index < 7
          ? `<div class="box square modal" image-id="${item.id}">
          <img src="${
            item.primaryImage
    ...
    <section class="carousel">
      ...
      ${
        index < 7
          ? `<div class="box square" image-id="${item.id}">
          <img src="${
            item.primaryImage
          }" alt="" class="model slides" image-id="${item.id}" />
          <img src="${
            item.secondaryImage
          }" alt="" class="model showcase" image-id="${item.id}" />
          <p class="marvel" image-id="${item.id}">${item.name}</p>
          <p class="price" image-id="${
            item.id
          }">&#8358; ${item.price.toLocaleString()}</p>
          ${
            item.quantity > 0
